...
- **Mismatch/Monte Carlo readiness:** Binding the `PROCESS_SWITCH=1` knob and zeroing the `sky130_fd_pr__*__slope*` mismatch factors keeps `Toxe/Toxp` positive with `MC_MM_SWITCH=1`, so the shared-sense deck now sweeps 50 seeds per 0.9/1.0/1.1 V corner while the new `sense_thresh_latency` probe timestamps the time it takes `sharedDriveDiff` to cross 0.899/0.900 V (pre-biased below the guard). The aggregated CSV (`logs/mismatch-mc/mismatch_mc.csv`) now records ≈0.125 ps per seed plus the 860–865 mV min headroom histogram, and the matching TT sweep (`logs/mismatch-mc-tt/mismatch_mc_tt.csv`) logs ≈4.75 ps with a 960–965 mV min. Copy those tuples along with the energy numbers into `STATUS.md`/`SUMMARY.md` so the periphery ledger always pairs headroom/jitter with energy before we claim a viable guard.
- **Mismatch/MC results:** Running 50 seeds per corner at 0.9/1.0/1.1 V now produces `Edyn ≈ 0.313 pJ` and `Eword_est ≈ 3.34 pJ` while `sense_headroom_min` stays around 860–865 mV and `sense_headroom_max` hovers near 900 mV; the resulting histogram (`logs/mismatch-mc/headroom_histogram.csv`) proves the ≥20 mV guard plus the new ≈125 fs jitter column already exist. The TT sweep now reports `Edyn ≈ −0.027 pJ`, `Eword_est ≈ −0.29 pJ`, headroom ≈0.998 V (`logs/mismatch-mc-tt/headroom_histogram.csv`), and ≈4.75 ps of jitter. The upsized deck (`DRIVER_WN=3u`, `DRIVER_WP=6u`) repeats the sweep with the same guard bins, so future logs can focus on latency/performance trade-offs rather than re-proving the guard.
