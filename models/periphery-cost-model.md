...
- **Mismatch/Monte Carlo readiness:** Binding the `PROCESS_SWITCH=1` knob and explicitly zeroing the `sky130_fd_pr__*__slope*` mismatch factors after the included parameter files keeps `Toxe/Toxp` positive even with `MC_MM_SWITCH=1`, so the shared-sense deck now runs with mismatch enabled (the only remaining limitations are the low swing itself, which still prevents `td_*`/`settle_*` from finding 0.5·VDD). Capture the energy/headroom tuples for 0.9 V/1.0 V/1.1 V corners now that the fatal is cleared.
- **Mismatch/MC results:** The base mismatch deck (with `DRIVER_WN=2u`, `DRIVER_WP=4u`) still produces `Edyn ≈ 0.313 pJ` / `0.452 pJ` and `Eword_est ≈ 3.37 pJ` / `4.86 pJ` while the driver-difference `sense_headroom_*` probes swing only ≈±0.3 mV at ±10% and occasionally touch ≈+24.6 mV at TT; those tuples live in `logs/mismatch-mc/mismatch_mc.csv` and `logs/mismatch-mc-tt/mismatch_mc_tt.csv`. A dedicated upsized deck (`spicemodels/option-b-encoder-with-shared-sense-mismatch-upsized.spice`, `DRIVER_WN=3u`, `DRIVER_WP=6u`) re-runs 50 seeds per corner with the same knobs and records almost identical energy but a slightly wider driver difference (≈±0.416 mV) in `logs/mismatch-mc-upsized/mismatch_mc.csv` via `tools/aggregate_mismatch_logs.py`. Every histogram still falls well short of the safety target (≈20 mV), so the periphery dominance ledger remains transparent about the missing noise margin before any level restoration or upsizing beyond 1.5× is contemplated.
