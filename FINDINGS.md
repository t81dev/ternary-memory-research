...
`tools/run_noise_mismatch_driver_sweep.sh` reparse now writes `sense_min/max` and `comp_pass` for every valid row in `logs/noise-mismatch-{5m,10m}-driver-{1p5,2,2p5}/mismatch_mc.csv` plus the canonical `logs/mismatch-mc/mismatch_mc.csv`, so the ledger explicitly captures that `comp_pass=failed` throughout the dataset (all 860–865 mV guard bins, 3.34 pJ `Eword_est`). The upsized mismatch logs (`logs/mismatch-mc-upsized/*`) abort early (`could not find a valid modelname`), so they were skipped but noted as warnings during the reparse, leaving the current ledger to document the unfired comparator before driver upsizing or a different topology is reconsidered.
